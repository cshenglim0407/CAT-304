...
INSERT INTO
    USERS (USERNAME, FIRST_NAME, LAST_NAME, DATE_OF_BIRTH, GENDER, EMAIL)
VALUES
('johndoe', 'John', 'Doe', '1990-01-15', 'MALE', 'johndoe@example.com'),
('janedoe', 'Jane', 'Doe', '1992-05-22', 'FEMALE', 'janedoe@example.com'),
('bobsmith', 'Bob', 'Smith', '1988-11-30', 'MALE', 'bobsmith@example.com');
...
('template_fingerprint_janedoe_device_002', 'v1.0', 'device_002', 'Fingerprint Scanner B', 'fingerprint',
    (SELECT USER_ID FROM USERS WHERE USERNAME = 'janedoe')),
('template_face_bobsmith_device_003', 'v1.0', 'device_003', 'Face Detector C', 'face',
    (SELECT USER_ID FROM USERS WHERE USERNAME = 'bobsmith'));
...
BEGIN
    SELECT USER_ID INTO v_user_id FROM USERS WHERE USERNAME = 'johndoe';
...
DECLARE
    ...
    v_account_id UUID;
BEGIN
    SELECT USER_ID INTO v_user_id FROM USERS WHERE USERNAME = 'janedoe';
...
DECLARE
    ...
    v_account_id UUID;
BEGIN
    SELECT USER_ID INTO v_user_id FROM USERS WHERE USERNAME = 'bobsmith';
...
VALUES
('BACHELORS', TRUE, TRUE, 2, 15000.00,
    (SELECT USER_ID FROM USERS WHERE USERNAME = 'johndoe')),
('MASTERS', TRUE, FALSE, 0, 5000.00,
    (SELECT USER_ID FROM USERS WHERE USERNAME = 'janedoe')),
('DIPLOMA', FALSE, TRUE, 1, 2000.00,
    (SELECT USER_ID FROM USERS WHERE USERNAME = 'bobsmith'));
